...
# TRAINING_QUERIES={key:{'query':file,'positives':[files],'negatives:[files], 'neighbors':[keys]}}
#TEST_QUERIES={key:{'query':file,'positives':[files],'negatives:[files], 'neighbors':[keys]}}
...
def train():
    ...
    LOG_FOUT.write("\n")
    LOG_FOUT.flush()           #todo: don't understand
...
def train_one_epoch(model, optimizer, train_writer, loss_function, epoch):
    ...
    #一个epoch训练 len(train_file_idxs)个query，batch_size是cfg.BATCH_NUM_QUERIES，所以循环len(train_file_idxs)//cfg.BATCH_NUM_QUERIES次
    for i in range(len(train_file_idxs)//cfg.BATCH_NUM_QUERIES):
        ...
        for j in range(cfg.BATCH_NUM_QUERIES):
            if (len(TRAINING_QUERIES[batch_keys[j]]["positives"]) < cfg.TRAIN_POSITIVES_PER_QUERY):
                #每个query如果没有足够个数的positive_query
                faulty_tuple = True
                break
...
def get_random_hard_negatives(query_vec, random_negs, num_to_take):
    ...
    Args:
        query_vec ([type]): [#,256]
        random_negs ([type]): negatives list
        num_to_take ([type]):  int 10
